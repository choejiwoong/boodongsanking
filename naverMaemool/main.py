...
# 페이지 설정: 항상 최상단에 위치
st.set_page_config(layout="wide", page_title="네이버 아파트 매물 리스트")

# 비밀번호 설정
PASSWORD = "chlwldnd"  # 원하는 비밀번호를 설정하세요
    ...
    if st.session_state.dataframe is not None:
        ...
        if apt_filter.strip():  # 공백을 제거하고 입력값이 있는 경우만 필터링
            filtered_df = filtered_df[
                filtered_df["아파트명"].str.contains(apt_filter.strip(), case=False, na=False)]
        # 면적 범위 필터링
        if area_filter == "~24평":
        ...
        # '면적' 컬럼을 "면적(평)" 형식으로 변경
        filtered_df["면적"] = filtered_df["면적"].apply(
            lambda x: f"{x / 3.3:.0f}평 ({x:.2f}m²)" if pd.notnull(x) else "N/A")
        # '면적_평' 컬럼 삭제
